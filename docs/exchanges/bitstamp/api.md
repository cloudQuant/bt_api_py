...
- 交易所类型: CEX (中心化交易所)
- 总部: 卢森堡（欧洲最老牌加密货币交易所之一，成立于 2011 年）
...
| X-Auth-Nonce | 36 位唯一字符串（UUID，150 秒内不可复用） |
...
- *参数**: `offset`, `limit` (最大 1000), `sort` (`asc`/`desc`)
