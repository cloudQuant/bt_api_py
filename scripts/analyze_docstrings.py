...
def quick_analyze(filepath: str) -> dict[str, Any]:
    ...
    try:
        ...
        public_methods = [
            m for m in ast_results["methods"] if m["is_public"] or m["name"] in important_dunders
        ]
...
def batch_analyze(root_dir: str, summary_only: bool = False, verbose: bool = False) -> None:
    ...
    if needs_work:
        ...
        needs_work.sort(
            key=lambda x: x.get("missing_docstrings", 0) + x.get("chinese_comments", 0),
            reverse=True,
        )
        ...
        for r in needs_work:
            ...
            print(
                f"{rel_path:<50} {r['total_lines']:>6} {r.get('missing_docstrings', 0):>8} {r.get('chinese_comments', 0):>8}"
            )
...
Examples:
  ...
  python scripts/analyze_docstrings.py --verbose
    ...
    parser.add_argument(
        "files", nargs="*", help="Python file(s) to analyze. If not specified, checks all files."
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Show detailed reports for files needing work"
    )
    parser.add_argument(
        "-s", "--summary", action="store_true", help="Only show summary table, no detailed reports"
    )
    parser.add_argument(
        "-d",
        "--directory",
