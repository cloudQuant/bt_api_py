...
class UpbitRequestDataSpot(UpbitRequestData):
    ...
    def get_depth(self, symbol, count=50, extra_data=None, **kwargs):
        path, params, extra_data = self._get_depth(symbol, count=count, extra_data=extra_data, **kwargs)
        return self.request(path, params=params, extra_data=extra_data, is_sign=False)
    ...
    async def async_get_trades(self, symbol, count=50, extra_data=None, **kwargs):
        return await self.async_get_trade_history(symbol, count=count, extra_data=extra_data, **kwargs)

    # ── make order ──────────────────────────────────────────────

    def make_order(self, symbol, size, price=None, order_type="bid-limit", extra_data=None, **kwargs):
        path, body, extra_data = self._make_order(symbol, size, price=price, order_type=order_type, extra_data=extra_data, **kwargs)
        return self.request(path, body=body, extra_data=extra_data, is_sign=True)

    async def async_make_order(self, symbol, size, price=None, order_type="bid-limit", extra_data=None, **kwargs):
        path, body, extra_data = self._make_order(symbol, size, price=price, order_type=order_type, extra_data=extra_data, **kwargs)
