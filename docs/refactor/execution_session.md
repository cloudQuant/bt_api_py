# 拆分方案卡：`bt_api_py/_execution_session.py`

> 职责定位：执行会话与恢复编排　|　现状 **8251 行**（迭代07 M5-Task14 产出，本轮仅出方案不改代码）
...
| 类 | 行数 | 方法数 |
|----|------|--------|
| `_ExecutionSession` | 6817 | 142 |
| `_WindowsLocker` | 6 | 1 |

| 顶层函数 | 行数 |
|---------|------|
| `migrate_execution_journal` | 361 |
| `session_config` | 103 |
| `_execution_arm_proof` | 87 |
| `_recover_migration_transaction` | 69 |
| `_claim_identity` | 39 |
| `_canonical_ctp_bundle_instrument` | 37 |
| `_semantic_identity_value` | 36 |
| `_canonical_ctp_instrument` | 34 |
| `_complete_migration_files` | 32 |
| `_recorded_identity_matches` | 27 |
