...
def example_get_sub_account_funding_balance():
    ...
    result = client.get_sub_account_funding_balance(sub_acct="your_sub_account_name")
...
def example_get_sub_account_max_withdrawal():
    ...
    result = client.get_sub_account_max_withdrawal(sub_acct="your_sub_account_name", ccy="USDT")

    if result.get_status():
        max_withdrawals = result.get_data()
        for item in max_withdrawals:
            print(f"  - Currency: {item.get('ccy')}, Max Withdrawal: {item.get('maxWd')}")
    else:
...
def example_create_sub_account():
    ...
    result = client.create_sub_account(sub_acct="new_sub_account_name", label="Trading Sub-account")
