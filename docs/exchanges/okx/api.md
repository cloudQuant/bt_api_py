...
- *参数**: `instId` (必需), `sz` (可选, 深度档数, 默认 1, 最大 400)
...
- *参数**: `instId` (必需), `limit` (可选, 默认 100, 最大 500)
...
| bar | STRING | 否 | K 线周期: 1m/3m/5m/15m/30m/1H/2H/4H/6H/12H/1D/1W/1M/3M (默认 1m) |

| after | STRING | 否 | 此时间之前的数据 (毫秒时间戳) |

| before | STRING | 否 | 此时间之后的数据 |

| limit | STRING | 否 | 数量 (默认 100, 最大 300) |
...
| `wss://ws.okx.com:8443/ws/v5/business` | 业务频道（K 线等） |
...
- 添加市场数据 API（Ticker、订单簿、成交、K 线、资金费率）
