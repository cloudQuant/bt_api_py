...
3. **功能验证**：确认 `client.py` 和 `live_ctp_feed.py` 无影响
...
2. **不动 C 扩展**：`_ctp.cpython-311-darwin.so` 完全不变
3. **不删 ctp.py**：保留为权威源，子模块从中导入
4. **100% 向后兼容**：所有现有导入路径不变
5. **自动化可重复**：CTP SDK 更新后重新运行脚本即可
