...
print(f"BTC/USD: 最新价={last}, 买一={bid}, 卖一={ask}, 24h成交量={volume}")
```
...
for order_id, price, amount in raw_book[:5]:
    print(f"订单ID={order_id}, 价格={price}, 数量={amount}")
```
...
**可选参数**:
- 在URL中添加交易对: `/v2/auth/r/orders/{symbol}`
...
for order in orders:
    ...
    print(f"订单ID={order_id}, {symbol}, 类型={order_type}, 价格={price}, 数量={amount}, 状态={status}")
...
**参数**:
- `id` (int): 订单ID
- 或 `cid` (int) + `cid_date` (string): 客户端订单ID + 日期 (YYYY-MM-DD)
...
Bitfinex 对REST API实施速率限制，以确保平台稳定性。

**限制范围**: 每分钟10-90次请求，具体取决于端点

**限制类型**:
- 公共端点: 较宽松的限制
- 认证端点: 根据端点类型有不同限制
- 订单相关: 基础限制为每5分钟1000个订单，根据交易量比例增加

**建议**:
- 实现请求队列和速率控制
- 使用WebSocket获取实时数据，减少REST请求
- 缓存不经常变化的数据
- 监控响应头中的速率限制信息
...
| 10020 | 维护中 | 等待维护结束 |
| 10050 | 无效符号 | 检查交易对格式 |
| 10100 | 认证失败 | 检查API密钥和签名 |
| 10114 | 无效API密钥 | 验证API密钥是否正确 |
| 11000 | 速率限制 | 降低请求频率 |
    ...
    if isinstance(response, list) and len(response) > 0:
        if response[0] == 'error':
            ...
            error_handlers = {
                ...
                10100: lambda: print("认证失败，请检查API密钥"),
...
def main():
    ...
    for order in orders:
        ...
        price = order[16]
        status = order[13]
        print(f"订单ID={order_id}, 数量={amount}, 价格={price}, 状态={status}")
...
- **API状态页**: https://status.bitfinex.com
- **Python SDK**: https://github.com/bitfinexcom/bitfinex-api-py
- **WebSocket文档**: https://docs.bitfinex.com/docs/ws-general
