...
**参数**: `pair` (必需, 可多个逗号分隔), `limit` (可选, 默认100)
...
**参数**: `pair` (必需), `offset` (可选), `limit` (可选, 默认100, 最大10000)
...
| withdraw_get_txid | POST | 获取提现TXID |
...
- 添加市场数据 API（币种、交易对设置、Ticker、订单簿、成交、K线）
