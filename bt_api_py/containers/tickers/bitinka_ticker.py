...
class BitinkaRequestTickerData(TickerData):
    ...
    def init_data(self) -> BitinkaRequestTickerData:
        ...
        if data:
            ...
            self.bid_price = parse_float(data.get("bid") or data.get("buy"))
            self.ask_price = parse_float(data.get("ask") or data.get("sell"))
            self.volume_24h = parse_float(
                data.get("volume") or data.get("vol") or data.get("v") or data.get("volume_24h")
            )
            self.high_24h = parse_float(data.get("high") or data.get("h") or data.get("high_24h"))
            self.low_24h = parse_float(data.get("low") or data.get("l") or data.get("low_24h"))
