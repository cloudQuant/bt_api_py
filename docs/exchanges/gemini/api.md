...
**参数**: `limit_bids` (可选, 默认50), `limit_asks` (可选, 默认50)
...
**参数**: `limit_trades` (可选, 默认50, 最大500), `since` (可选, 时间戳), `include_breaks` (可选)
...
**参数**: `symbol` (必需), `limit_trades` (可选, 默认50, 最大500), `timestamp` (可选)
...
- 添加市场数据 API（Ticker V2、订单簿、成交、K线）详细说明
