...
def analyze_tests(tests: List[Dict[str, Any]], threshold: float = 1.0) -> None:
    ...
    print(f"\n{'='*80}")
    print(f"Test Performance Analysis")
    print(f"{'='*80}\n")
    ...
    network_tests = [t for t in slow_tests if "network" in t["nodeid"].lower() 
                     or "request" in t["nodeid"].lower() or "wss" in t["nodeid"].lower()]
    if network_tests:
