...
- 子仓 `ruff lint` 与 `ruff format` 已拆为各自的 report-only CI 矩阵任务；当前 10 项 lint 与 453 个待格式化文件（快照基线 452，OKX handoff 候选 87 > 86）清零、稳定 CI 且可独立回滚前，两者均不得升级为 blocking；详见 [`TECH_DEBT.md`](TECH_DEBT.md) N-06/N-18。
