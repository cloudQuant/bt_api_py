...
def test_okx_wss_data_feed():
    ...
    receive_okx_funding_rate_data = False
    ...
    while True:
        ...
        if isinstance(data, OkxFundingRateData):
            receive_okx_funding_rate_data = True
        if isinstance(data, OkxMarkPriceData):
            receive_okx_mark_price_data = True

    if not (
        receive_okx_bar_data
        and receive_okx_ticker_data
        and receive_okx_order_book_data
        and receive_okx_funding_rate_data
        and receive_okx_mark_price_data
    ):
        pytest.skip("Skipped (no data or incomplete data, likely network): OKX swap websocket feed")
    assert receive_okx_bar_data is True
    assert receive_okx_ticker_data is True
    assert receive_okx_order_book_data is True
    assert receive_okx_funding_rate_data is True
...
def test_get_okx_account_data_feed():
    ...
    while lots * ask_price < 10:
        lots += 1
    sell_data = live_okx_swap_feed.make_order(
        "OP-USDT", lots, ask_price, "sell-limit", client_order_id=sell_client_order_id
    )
    print(sell_data)
    data = live_okx_swap_feed.query_order("OP-USDT", client_order_id=sell_client_order_id)
    print("下单数据", data.get_data())
    data = live_okx_swap_feed.cancel_order("OP-USDT", client_order_id=sell_client_order_id)
