...
**参数**: `scale` (可选, 精度), `limit` (可选, 默认10, 最大150)
...
**参数**: `symbol` (可选), `side` (可选), `limit` (可选, 默认100)
