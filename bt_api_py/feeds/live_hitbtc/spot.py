...
class HitBtcSpotRequestData(HitBtcRequestData):
    ...
    def make_order(self, symbol, amount, price=None, order_type="buy-limit", extra_data=None, **kwargs):
        path, body, extra_data = self._make_order(symbol, amount, price=price, order_type=order_type, extra_data=extra_data, **kwargs)
    ...
    def async_make_order(self, symbol, amount, price=None, order_type="buy-limit", extra_data=None, **kwargs):
        path, body, extra_data = self._make_order(symbol, amount, price=price, order_type=order_type, extra_data=extra_data, **kwargs)
        self.submit(self.async_request(path, body=body, extra_data=extra_data, is_sign=True), self.async_callback)
