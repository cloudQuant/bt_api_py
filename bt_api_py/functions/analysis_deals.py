...
bit_df["qty"] = np.where(
    bit_df["side"] == "buy", bit_df["vol"] + bit_df["fee"] / bit_df["price"], -1 * bit_df["vol"]
)
...
for count, (_index, row) in enumerate(okex_df.iterrows(), start=1):
    ...
    print(
        f"第{count}次对冲订单，total_base_qty = {total_base_qty},hedge_ts = {hedge_ts}, qty={qty_sum}, hedge_qty = {hedge_qty}"
    )
    print("buy_df\n", buy_df)
    print("sell_df\n", sell_df)
    print("df\n", df)
    ...
    if qty_sum > 0:
        ...
        if len(sell_df) > 0:
            ...
            self_result.append(
                [count, "buy", buy_avg_price, "sell", sell_avg_price, total_fee, profit, net_profit]
            )
            # 剩余部分的量、价格、额和手续费
...
new_df["profit"] = np.where(
    new_df["bit_side"] == "buy",
    (new_df["okex_price"] - new_df["bit_price"]) * new_df["bit_vol"]
    + new_df["bit_fee"]
    + new_df["okex_fee"],
    (new_df["bit_price"] - new_df["okex_price"]) * new_df["bit_vol"]
    + new_df["bit_fee"]
    + new_df["okex_fee"],
)
print("new_df", new_df)
