...
from bt_api_py.config_loader import load_exchange_config, VenueType, ConnectionType
...
class TestBinanceCapabilities:
    def test_has_market_data_caps(self):
        caps = self._get_caps()
        for c in [Capability.GET_TICK, Capability.GET_DEPTH, Capability.GET_KLINE,
                   Capability.GET_FUNDING_RATE, Capability.GET_MARK_PRICE]:
            assert c in caps, f"Missing {c}"

    def test_has_trading_caps(self):
        caps = self._get_caps()
        for c in [Capability.MAKE_ORDER, Capability.CANCEL_ORDER,
...
class TestCtpCapabilities:
    def test_has_basic_trading_caps(self):
        caps = self._get_caps()
        for c in [Capability.MAKE_ORDER, Capability.CANCEL_ORDER,
                   Capability.GET_ACCOUNT, Capability.GET_POSITION]:
            assert c in caps
...
class TestProtocolCompliance:
    ...
    def test_binance_has_required_methods(self):
        from bt_api_py.feeds.live_binance.request_base import BinanceRequestData
        required = ['get_account', 'get_balance', 'get_tick', 'get_depth',
                     'get_kline', 'make_order', 'cancel_order', 'query_order',
                     'get_open_orders', 'get_deals', 'get_position',
                     'connect', 'disconnect', 'translate_error']
    ...
    def test_ctp_has_required_methods(self):
        from bt_api_py.feeds.live_ctp_feed import CtpRequestData
        required = ['get_account', 'get_balance', 'make_order', 'cancel_order',
                     'query_order', 'get_position', 'connect', 'disconnect',
                     'translate_error']
        for method in required:
