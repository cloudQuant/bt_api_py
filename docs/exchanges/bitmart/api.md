...
**参数**: `symbol` (必需), `limit` (可选, 默认35, 最大50)
...
**参数**: `symbol` (必需), `limit` (可选, 默认50, 最大50)
...
| after | LONG | 否 | 结束时间（秒） |
| step | INT | 否 | K线周期(分钟): 1,3,5,15,30,45,60,120,180,240,1440,10080,43200 |
| limit | INT | 否 | 数量限制 |
...
- `spot/kline1m:{symbol}` - K线
- `spot/depth5:{symbol}` - 深度（5档）
- `spot/depth20:{symbol}` - 深度（20档）
- `spot/depth50:{symbol}` - 深度（50档）
