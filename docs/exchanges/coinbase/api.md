...
- 文档版本: 1.0.0
- API 版本: v2 (REST), v3 (Advanced Trade)
- 创建日期: 2026-02-27
- 最后更新: 2026-02-27
...
Coinbase 提供多个 API 产品：

| API 类型 | URL | 说明 |

|---------|-----|------|

| Advanced Trade API | `<https://api.coinbase.com/api/v3`> | 高级交易 API（推荐） |

| Exchange API | `<https://api.exchange.coinbase.com`> | Coinbase Pro API |

| Wallet API | `<https://api.coinbase.com/v2`> | 钱包 API |
...
4. 设置以下信息：
   - API Key Name
   - 选择权限范围
1. 保存 Private Key（仅显示一次）
2. 可选：设置 IP 白名单

### 请求签名方法（Advanced Trade API）

Coinbase 使用 JWT（JSON Web Token）认证。
...
| product_ids | String | 否 | 产品 ID 列表（逗号分隔） |
...
| product_id | String | 是 | 产品 ID，如 BTC-USD |
...
start_time = end_time - 86400  # 24 小时前
...
| client_order_id | String | 是 | 客户自定义订单 ID（UUID 格式） |
...
| 私有端点 | 15 次 | 1 秒 | 每个 API 密钥 |

| 交易端点 | 10 次 | 1 秒 | 每个 API 密钥 |
...
Coinbase WebSocket 使用 heartbeat 消息：
...
| UNAUTHENTICATED | Authentication failed | 认证失败 | 检查 JWT token |

| PERMISSION_DENIED | Permission denied | 权限不足 | 检查 API 权限 |
...
| UNKNOWN_ORDER | Order not found | 订单不存在 | 检查订单 ID |
...
- 2026-02-27: 初始版本创建，基于 Coinbase Advanced Trade API v3
