...
class BudaRequestData(Feed):
    ...
    def __init__(self, data_queue, **kwargs):
        ...
        self.request_logger = SpdLogManager(
            "./logs/buda_feed.log", "request", 0, 0, False
        ).create_logger()
        self.async_logger = SpdLogManager(
            "./logs/buda_feed.log", "async_request", 0, 0, False
        ).create_logger()
        self._http_client = HttpClient(venue=self.exchange_name, timeout=10)

    def _generate_signature(self, timestamp, method, request_path, body=""):
        """Generate HMAC SHA256 signature for Buda API.

        Buda signature format:
        signature = HMAC-SHA256(timestamp + method + request_path + body)
        """
        secret = self._params.api_secret
        if secret:
            # Build signature string: timestamp + method + request_path + body
            if body:
    ...
    async def async_request(self, path, params=None, body=None, extra_data=None, timeout=5):
        """Async HTTP request for Buda API."""
        method = path.split()[0] if " " in path else "GET"
        request_path = path.split()[1] if " " in path else path
