...
def _get_coincheck_config() -> Any | None:
    ...
    try:
        config_path = get_exchange_config_path("coincheck.yaml")
        if config_path.exists():
            _coincheck_config = load_exchange_config(str(config_path))
        _coincheck_config_loaded = True
    except (OSError, ValueError, KeyError, ImportError) as e:
        logger.warning(f"Failed to load coincheck.yaml config: {e}")
    return _coincheck_config
